...
def get_frames_path_train(data_root):
    ...
    for i in range(train_idx_1, stop_idx_1):
        ...
        for ii in frame:
            ii_num = int(ii.split('.')[0])    # start=0, original image: 250x250
...
def train(x, num_epoch):
    ...
    x_c1 = x[0]                     # t    batchsize*3*imagewidth*imagewidth
    x_c2 = x[1]                     # t+k  k不一定为正
    x_c3 = x[random.randint(0,3)]   # td
    ...
    h_p1 = netEP(x_p1)
    h_p2 = netEP(x_p2).detach()  # used for scene discriminator
...
def train_scene_discriminator(x):
    ...
    acc =out[:half].le(0.5).sum() + out[half:].gt(0.5).sum()   # gt:大于，le 小于等于
...
for epoch in range(niter):
    ...
    for i, (videos, index) in enumerate(train_loader, start=1):   # i 从 1 开始
        torch.cuda.empty_cache()
        batchsize = len(videos)
        videos = Variable(videos)

        # videos = videos.view(x, batchsize, 3, opt.image_width, opt.image_width)
