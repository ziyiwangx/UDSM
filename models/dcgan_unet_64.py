...
class decoder(nn.Module):
    ...
    def forward(self, input):
        ...
        #print('content', len(content), len(content[0]))  # 3,4,10,12
        #print('h_c', len(pose), len(pose[0]))  # 4,128,12,12
        d1 = self.upc1(torch.cat([content, pose], dim=1))
        d2 = self.upc2(torch.cat([d1, skip[3]], dim=1))
        d3 = self.upc3(torch.cat([d2, skip[2]], dim=1))
        d4 = self.upc4(torch.cat([d3, skip[1]], dim=1))
